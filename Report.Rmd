...
# Occupation_Type
p_Och <- ggplot(app_mw, aes(x = Occupation_Type)) + geom_bar() +
  xlab("Occupation Type") + ylab("Application Numbers") +
  theme(axis.text.x = element_text(angle = 25, hjust = 0.5, vjust = 0.5))
...
```{r echo=FALSE,fig.show='hold',out.height='80%',out.width = '45%', fig.align = 'center',fig.cap = "Application Numbers and Target under different Occupation Type"}
p_Och
p_Occ
```
...
Then come to take a look at the risk grades of different Family Status and Accompany Situations. (Figure 2)
```{r echo=FALSE, out.height='90%',out.width = '80%',fig.align = 'center',fig.cap = "Target of different Family Status and Accompany"}
p_Fam
```
...
$Pr(Target_{i} = 1) = logit^{-1}(\alpha_{j[i]} + \beta^{Car} * Car + \beta^{Realty} * Realty + \beta^{Housing Type} * Housing Type)$, for $i = 1,...,n$  
...
They looks fit well. Almost all of the points are in the line.
...
First, from the results, *the occupations showed different effects on their risk evaluation, while accompany does not.* Also, we can guess that loan company is *more welcome Cleaning staff, Private service staff and Secretaries who live in co-op apartment, own their car. Also company may more open to married people and widow. *
